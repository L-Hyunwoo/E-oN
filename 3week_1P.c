...
int main(void)
{
	...
	overlap_x = term(x1[0], x1[1], x2[0], x2[1]);	//x좌표 비교
	overlap_y = term(y1[0], y1[1], y2[0], y2[1]);	//y좌표 비교

	if ((overlap_x * overlap_y) == 1)		//x,y좌표 둘다 1일때 overlap 출력
	{
		printf("Overlap");
	}
	else		//그 외에 No overlap 출력
	{
		printf("No overlap");
	}
	return 0;
